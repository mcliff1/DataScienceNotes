...
The goal is given a set of observations in the space $\left( \mathbb{X}_1 \times \dots \times \mathbb{X}_n \right)$ to come up with a candidate function *f* defined $f: \mathbb{X}_1 \times \mathbb{X}_2\times \dots \times \mathbb{X}_n\mapsto \mathbb{Y}$ that *matches* our training data as best we can and the function can be used to predict the dependent variable for future observations.
...
Regression with Regularization [glmnet](https://cran.r-project.org/web/packages/glmnet/glmnet.pdf)
...
### Gradient Boosting
Gradient Boosting [gbm](https://cran.r-project.org/web/packages/gbm/gbm.pdf)
...
#table(predict.glmnet, testing$status)
...
confMat.nnet
```
Looks like this is all showing negative outcomes
...
confMat.nnet <- confusionMatrix(predict.ksvm, testing$status)
confMat.nnet
```
...
length(predict.qda)
dim(testing)
#table(predict.qda, testing$status)
```
