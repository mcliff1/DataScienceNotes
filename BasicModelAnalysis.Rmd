...
c=ifelse( abs(y - 2*x) > .2, ifelse(y>2*x, "red", "blue"), ifelse(rbinom(1,1,.5), "red", "blue"))
demo_df = data.frame(x=x, y=y, c=c)

predict_df<- data.frame(pred=predict(lm(y~x, data=demo_df), demo_df), 
ggplot(data=demo_df, aes(x=x, y=y, col=c)) +
    geom_point()
```
...
Solves to problem where all the spaces are fields ($\mathbb{R}$), for classification problems they need to be converted to **one-hot** vectors, which can lead to a large **sparse matrix**. 
